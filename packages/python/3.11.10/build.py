...
def build(source_path, build_path, install_path, targets):
    package_url = 'https://www.python.org/ftp/python/3.11.10/Python-3.11.10.tar.xz'
    ...
    def _build():
        ...
        if not os.path.exists(makefile_path):
            ...
            configure_proc = subprocess.Popen('../{}/configure --prefix=$REZ_BUILD_INSTALL_PATH --with-lto -with-openssl=$REZ_OPENSSL_ROOT --without-pymalloc --enable-optimizations CPPFLAGS=-I$REZ_ZLIB_ROOT/include LDFLAGS=-L$REZ_ZLIB_ROOT/lib'.format(folder_name), shell=True)
    ...
    def _install():
        ...
        os.symlink(os.path.join(install_dir, 'bin', 'python3'),
                   os.path.join(install_dir, 'bin', 'python'))
        os.symlink(os.path.join(install_dir, 'bin', 'pip3'),
                   os.path.join(install_dir, 'bin', 'pip'))
        print('install done')
